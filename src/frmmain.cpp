...
frmMain::frmMain(QWidget *parent): QMainWindow(parent), ui(new Ui::frmMain){
    ...
    QApplication::setApplicationVersion("0.94");
...
void frmMain::szinez(){
    ...
    foreach (QGraphicsItem *item, scene->selectedItems()) {
        ...
        if (s==6){
            ...
            l->setPen(QPen(QColor(cb->currentText()), 1.5, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
        }
    }
}
...
void frmMain::rotate(){
    ...
    //scene->addPixmap(pixMap.transformed(trans));
    ...
    //scene->clear();
    //QPixmap pixt = pix.transformed(trans);

    //scene->addPixmap(pixt);


    //scene = gView->scene();

    //gView->setRenderHint(QPainter::Antialiasing);
    //gView->setRenderHint(QPainter::SmoothPixmapTransform);
...
//    gView->resetMatrix();
    ...
    //scene->addPixmap(pix.transformed(trans));
...
void frmMain::furesz(){
    ...
    if (scene->selectedItems().count()!=1 or s!=5){
        QMessageBox msgBox;
        msgBox.setIcon(QMessageBox::Information);
        msgBox.setText("Figyelmezzünk!");
        ...
        if(n==0){
            scene->addKorB(scene->selectedItems().first()->x(), scene->selectedItems().first()->y(), 1);
        }
...
//        scene->addKorB(scene->selectedItems().first()->x(), scene->selectedItems().first()->y(), ui->comboBox_2->currentText().toDouble());
    }
...
void frmMain::elong(){
    ...
    if (scene->selectedItems().count()!=1 or s!=6){
        ...
        scene->addVonalB(A.x(), A.y(), B.x(), B.y(), QColor(cb->currentText()));
    }
}
...
void frmMain::calib(){
    ...
    if (scene->selectedItems().count()!=1 or s!=6){
        ...
        l->setData(0, "calib");
        // itt jó lenne kitörölni a két pontot, ami a vonalat definiálta
        //scene->setVonal();
//        qDebug() << scene->dPix;
...
void frmMain::hossza(){
    ...
    if (scene->selectedItems().count()!=1 or s!=6){
        ...
        QGraphicsLineItem *l = qgraphicsitem_cast<QGraphicsLineItem *>(scene->selectedItems().first());
        double h=0;
        //h = (l->line().length()/scene->dPix)*ui->doubleSpinBox_3->value();
        h = (l->line().length()/scene->dPix)*mertek->value();
        //ui->textEdit->append(QString("Hossza: %1 mm").arg(QString::number(h,'f',1)));
...
//        QLineF l3(0.0, pont.y(), pont.x(), pont.y());

//        QPointF* ip= new QPointF;
//        l1.intersect(l3, ip);

//        QLineF l4(ip->x(), ip->y(), pont.x(), pont.y());
//        QLineF l5(pont.x(), pont.y(), 0.0, pont.y());
////        qDebug() << l5.length();
//        l5.setLength(tav);

////        QPointF* tp= new QPointF;
////        qDebug() << l5.length();
////        l5.setAngle(90+(180-l1.angleTo(l4)));

////        double d1 = sqrt(pow(l1.p1().x()-l5.p1().x(),2) + pow(l1.p1().y()-l5.p1().y(),2));
////        double d2 = sqrt(pow(l1.p1().x()-l5.p2().x(),2) + pow(l1.p1().y()-l5.p2().y(),2));
////        if(d2>d1){
////            l5.setAngle(90+(180-l1.angleTo(l4))-180);
////        } else {
////            l5.setAngle(90+(180-l1.angleTo(l4)));
////        }
////        qDebug() << d1 << d2;
...
//        qDebug() << l5.length();
//        scene->addVonalB(l4.p1().x(), l4.p1().y(), l4.p2().x(), l4.p2().y());

//        QLineF la(pont.x(), pont.y(), l1.p1().x(), l1.p1().y());
//        QLineF lb(pont.x(), pont.y(), l1.p2().x(), l1.p2().y());
...
//        qDebug() << b << l1.length();
        ...
        scene->addVonalB(l5.p1().x(), l5.p1().y(), l5.p2().x(), l5.p2().y(), QColor(cb->currentText()));
//        scene->addPontB(ip->x(), ip->y());


//        QLineF l4(pont.x(), pont.x(), TPx.at(0), TPy.at(0));

//        QLineF l5(pp.x(), pp.y(), 0.0, pp.y());
////        l5.setLength(tav);
////        qDebug() << tav;
////        l5.setAngle(90+(180-l2.angleTo(l4)));
//        scene->addVonalB(l5.p1().x(), l5.p1().y(), l5.p2().x(), l5.p2().y());
//        scene->addVonalB(0, 0, 500, 500);
//        qDebug() << l5.p1().x()<< l5.p1().y()<< l5.p2().x()<< l5.p2().y();
    }
}
...
void frmMain::szogmero(){
    ...
    if(lns.length()==2){
        ...
        if(ba>bb){
    //        ui->textEdit->append(QString("Külső szög: %1\nBelső szög: %2").arg(ba).arg(bb));
    ...
    //        ui->textEdit->append(QString("Külső szög: %1\nBelső szög: %2").arg(bb).arg(ba));
        ...
        if(ba==bb){
    //        ui->textEdit->append(QString("Szög: %1").arg(ba));
...
void frmMain::pontosszekoto(){
    ...
    if (scene->selectedItems().count()!=2){
        ...
        scene->addVonalB(xs.at(0), ys.at(0), xs.at(1), ys.at(1), QColor(cb->currentText()));
    }
}
...
//    rtgFile = "/home/sn/dev/aote/VOM/figs/e-26199-02.jpg";
    scene->clear();
    gView->resetMatrix();
